...
// Notes are stored per user: noto_notes_<userId>
...
export default function Dashboard() {
  ...
  const T = {
    ...
    cardHoverBdr: dark ? "rgba(124,58,237,0.3)" : "rgba(124,58,237,0.4)",
  ...
  async function deleteNote(id, e) {
    ...
    setNotes(updated);
    setFiltered(updated.filter(n => !activeTag || (n.tags || []).includes(activeTag)));
    ...
    <Layout>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "14px", marginBottom: "32px" }} className="stagger">
        {[
          ...
          <div key={s.label} className="fade-in" style={{ background: T.card, border: `1px solid ${T.cardBdr}`, borderRadius: "14px", padding: "18px 20px", position: "relative", overflow: "hidden" }}
            ...
            <div style={{ fontSize: "30px", fontWeight: 800, color: s.accent, fontFamily: "monospace", lineHeight: 1 }}>{s.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ position: "relative", marginBottom: "20px" }} className="fade-in">
        ...
        <input type="text" placeholder="Search notes…" value={search} onChange={e => setSearch(e.target.value)}
          style={{ width: "100%", background: T.search, border: `1px solid ${T.searchBdr}`, borderRadius: "12px", paddingLeft: "40px", paddingRight: "16px", paddingTop: "11px", paddingBottom: "11px", fontSize: "14px", color: T.text, outline: "none", boxSizing: "border-box", fontFamily: "monospace" }}
          ...
          {["All", ...allTags].map(tag => {
            ...
            return (
              <button key={tag} onClick={() => setActiveTag(isAll ? null : activeTag === tag ? null : tag)}
                style={{ fontSize: "11px", padding: "5px 12px", borderRadius: "20px", border: `1px solid ${active ? "rgba(124,58,237,0.4)" : T.cardBdr}`, background: active ? "rgba(124,58,237,0.15)" : "transparent", color: active ? "#a78bfa" : T.text2, cursor: "pointer", fontFamily: "monospace", transition: "all 0.15s" }}>
                {isAll ? "All" : `#${tag}`}
              </button>
            );
          })}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: "14px" }} className="stagger">
        ...
        {loading
          ? [1, 2, 3].map(i => <div key={i} className="shimmer fade-in" style={{ borderRadius: "16px", height: "160px", border: `1px solid ${T.cardBdr}` }} />)
              ...
              return (
                <div key={note.id} className="fade-in note-card" style={{ position: "relative", animationDelay: `${idx * 40}ms` }}
                  ...
                  <Link to={`/note/edit/${note.id}`} style={{ textDecoration: "none" }}>
                    <div style={{ background: T.card, border: `1px solid ${T.cardBdr}`, borderRadius: "16px", padding: "18px", minHeight: "160px", display: "flex", flexDirection: "column", cursor: "pointer", overflow: "hidden", position: "relative", transition: "all 0.2s" }}
                      onMouseEnter={e => { e.currentTarget.style.borderColor=T.cardHoverBdr; e.currentTarget.style.background=dark?"#0d0f1a":"#f5f3ff"; }}
                      onMouseLeave={e => { e.currentTarget.style.borderColor=T.cardBdr; e.currentTarget.style.background=T.card; }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: "14px", flexWrap: "wrap", gap: "6px" }}>
                        <div style={{ display: "flex", gap: "5px", flexWrap: "wrap" }}>
                          {(note.tags || []).slice(0, 2).map(t => (
                            <span key={t} style={{ fontSize: "10px", background: "rgba(124,58,237,0.1)", color: T.tagC, border: "1px solid rgba(124,58,237,0.2)", padding: "2px 8px", borderRadius: "20px", fontFamily: "monospace" }}>#{t}</span>
                          ))}
                        </div>
                        <span style={{ fontSize: "11px", color: T.text3, fontFamily: "monospace" }}>{time ? timeSince(time) : ""}</span>
                      </div>
                    </div>
                  </Link>
      ...
      {!loading && filtered.length === 0 && (
        <div className="fade-in" style={{ textAlign: "center", padding: "80px 20px", color: T.text2 }}>
          ...
          <div style={{ fontSize: "14px" }}>{search ? `No notes match "${search}"` : "No notes yet — create your first one!"}</div>
        </div>
      )}
    </Layout>
  );
